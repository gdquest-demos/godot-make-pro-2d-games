...
func _change_state(new_state):
	...
	match new_state:
		...
		PREPARE_TO_CHARGE:
			...
			$Timer.start()
		CHARGE:
			charge_direction = (target_position - position).normalized()
			charge_distance = 0.0
	...
	match current_state:
		...
		ROAM:
			...
			if position.distance_to(roam_target_position) < ARRIVE_DISTANCE:
				...
				_change_state(SPOT)
		RETURN:
			velocity = arrive_to(velocity, start_position, roam_slow_radius, max_roam_speed, true)
			move_and_slide(velocity)
		...
		FOLLOW:
			...
			if position.distance_to(target_position) < CHARGE_RANGE:
